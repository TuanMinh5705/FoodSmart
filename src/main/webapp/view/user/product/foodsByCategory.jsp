       ...
       <c:when test="${not empty storeList}">
           <c:forEach var="store" items="${storeList}">
               <a href="/homeUser?action=showStore&store_id=${store.store_id}" class="block border rounded-2xl p-4 shadow-lg hover:shadow-xl transition">
                   ...
                   <p class="text-2xl font-semibold text-center mt-4">${store.store_name}</p>
               </a>
           </c:forEach>
       </c:when>
       <c:otherwise>
                ...
                fetch(`/homeUser?action=toggleFavorite&food_id=${productId}`, {
                    ...
                    .then(data => {
                        if (data.trim() === "added") {
                            ...
                            this.innerHTML = '<i class="bi bi-heart-fill"></i>'; // Chuyển sang biểu tượng trái tim đầy
                        } else if (data.trim() === "removed") {
                            this.classList.remove("bg-red-500", "text-white");
                            this.innerHTML = '<i class="bi bi-heart"></i>'; // Chuyển lại trái tim rỗng
                        }
                    })
