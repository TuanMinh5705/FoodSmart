...
<!-- Phần Cửa hàng -->
<h3 class="text-2xl font-bold mt-4">Cửa hàng</h3>
<div class="px-4">
            ...
            <c:when test="${not empty storeList}">
                <c:forEach var="store" items="${storeList}">
                    <a href="/homeUser?action=showStore&store_id=${store.store_id}" class="block border rounded-2xl p-4 shadow-lg hover:shadow-xl transition">
                        <img src="${pageContext.request.contextPath}/foodSmartImages/avatars/${store.avt_path}" alt="Ảnh đại diện cửa hàng" class="w-24 h-24 object-cover rounded-full mx-auto">
...
<!-- Phần Sản phẩm -->
<h3 class="text-2xl font-bold mt-4">Sản phẩm</h3>
<div class="px-4">
    <div class="grid grid-cols-4 gap-4 mt-4">
        <c:choose>
            <c:when test="${not empty foodList}">
                <c:forEach var="food" items="${foodList}">
                    <div class="bg-white p-4 rounded shadow-md border border-gray-300 relative group">
                        <div class="relative">
                            <img src="${not empty food.list_food_images ? pageContext.request.contextPath + '/foodSmartImages/product/' + food.list_food_images[0].image_path : '../../../images/product_default.png'}"
                                 alt="${food.product_name}" class="w-40 h-40 object-contain mx-auto">
                            <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button class="favorite-btn bg-gray-200 text-gray-600 p-4 rounded-full hover:bg-red-500 hover:text-white" data-product-id="${food.product_id}">
                                    <i class="bi bi-heart"></i>
                                </button>
                                <button class="bg-gray-200 text-gray-600 p-4 rounded-full hover:bg-blue-500 hover:text-white" onclick="addToCart(${food.product_id}, ${food.price})">
                                    <i class="bi bi-cart"></i>
                                </button>
                                <a href="/homeUser?action=showFoodDetail&id=${food.product_id}" class="bg-gray-200 text-gray-600 p-4 rounded-full hover:bg-green-500 hover:text-white">
                                    <i class="bi bi-eye"></i>
                                </a>
                            </div>
                        </div>
                        <div class="mt-2">
                            ...
                            <a href="/homeUser?action=buyNow&id=${food.product_id}">
                                <button onclick="buyNow(${food.product_id}, ${food.price})" class="bg-orange-500 text-white py-2 px-4 rounded hover:bg-orange-600 transition">
    ...
    document.addEventListener("DOMContentLoaded", function () {
        document.querySelectorAll(".favorite-btn").forEach(button => {
            button.addEventListener("click", function () {
                ...
                fetch(`/homeUser?action=toggleFavorite&food_id=${productId}`)
